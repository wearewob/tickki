...
export default function Feature3() {
    ...
    useEffect(() => {
        ...
        const playSequence = () => {
            ...
            const sequence = [
                { step: 1, delay: 500 },  // Show button in center
                { step: 2, delay: 1500 }, // Card pops out, button moves to bottom left
                { step: 3, delay: 6000 }, // Wait, then reset loop
            ];
    ...
    return (
        <section
            ref={sectionRef}
            className="w-full h-(--height-feature-section) bg-surface flex items-center justify-center px-6 md:px-36"
        ...
        >
            <div className="max-w-6xl w-full flex flex-col lg:flex-row items-center gap-16">
                ...
                <div className="flex-1 w-full h-(--height-feature-card) rounded-[24px] bg-surface-light flex items-center justify-center p-12 relative isolate">
                    <div className="w-[365px] flex flex-col items-center justify-end relative h-full">

                        {/* The Chat Card (Hidden initially, pops out in Step 2) */}
                        <div
                            className={`w-full h-[335px] shadow-card rounded-2xl bg-surface border border-border-warm flex flex-col overflow-hidden absolute bottom-[60px] transition-all duration-700 ease-in-out origin-center z-[1] ${step >= 2 ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-[0.3] translate-y-20 pointer-events-none"
                                }`}
                        >
                            {/* Header */}
                                ...
                                <div className="flex flex-col items-start gap-1">
                                    ...
                                    <div className="flex items-start gap-2">
                                        <div
                                            ...
                                            aria-hidden="true"
                                        />
                                        <div className="rounded-xl rounded-tl-sm bg-surface py-2 px-3 shadow-chat-bubble border border-border-warm/50 max-w-[260px]">
                                            <p className="text-[11px] font-body leading-snug text-brand-dark">
                                                Hi there,
                                                <br />
                            ...
                            <div className="bg-surface border-t border-surface-muted p-4 flex flex-col gap-3">
                                <div className="rounded-2xl bg-surface-light p-4 flex flex-col gap-4">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <button className="text-[11px] text-text-muted flex items-center gap-1.5 hover:text-brand-dark transition-colors font-medium">
                                ...
                                <p className="text-center text-[9px] text-text-muted font-body">
                                    Powered by Tickki
                                </p>
                            </div>
                        </div>

                        {/* WhatsApp Badge (Visible in Step 1, acts as the anchor) */}
                        <div
                            className={`shadow-card rounded-full bg-whatsapp py-2.5 px-6 h-[48px] flex items-center justify-center gap-2 absolute z-[2] transition-all duration-700 ease-in-out ${step >= 2
                                ? "bottom-[-16px] left-[50%] -translate-x-1/2 opacity-100 scale-100"
                                : step === 1
                                    ? "bottom-[50%] left-[50%] -translate-x-1/2 translate-y-1/2 opacity-100 scale-100"
                                    : "bottom-[50%] left-[50%] -translate-x-1/2 translate-y-1/2 opacity-0 scale-50"
                                }`}
                        >
                            <svg className="w-6 h-6 text-surface shrink-0" fill="currentColor" viewBox="0 0 24 24">
